...
# 数据集文件夹位置
filepath = r"D:\学习\大创\data\训练数据集\data\Track1+CoughVid(增强)(mel)(cough_decated)"
# 权重文件位置
...
# 写一个txt文件用于保存超参数
file = open(photo_folder + '\\' + 'covnet网络 ' + nowTime + ".txt", 'w',encoding='utf-8')
...
for train_val_both, test_index in kf.split(data_0):
        ...
        with torch.no_grad():  # 下面不进行梯度计算
            ...
            if val_loss[-1] >= min_val_loss + 0.1:
                if (epoch + 1) - epoch_num >= 4:
                    # 保存的权重名称
                    savename = pth_flodr + '\\model_' + dir_path + "_第{}折验证covnet网络".format(k_num) + '.pth'
                    # 保存当前权重
            ...
            if acc_val > best_acc_all:
                ...
                # 保存的权重名称
                savename = pth_flodr + '\\model_' + dir_path + '最好的权重{}层网络'.format(net_num) + '.pth'
                # 保存当前权重
                torch.save(net.state_dict(), savename)
    ...
    plt.savefig(photo_folder + "\\model_loss_第{}折_".format(k_num) + str(nowTime) + ".jpg")
    # plt.show()
    # plt.xlim((0,50))
    # plt.ylim((0,1))
    plt.figure()
...
plt.savefig(photo_folder + "\\model_ROC_" + str(nowTime) + ".jpg")
plt.show()

"""
绘制混淆矩阵，并保存
"""
Confusion_matrix_path = photo_folder + "\\Confusion matrix" + str(nowTime) + ".jpg"
