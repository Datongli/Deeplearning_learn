...
# -------------------------------------------------- #
# 计算图片的总数量
negative_path = os.path.join(filepath_train_val_1, negative)
positive_path = os.path.join(filepath_train_val_1, positive)
...
for train_index, val_index in kf.split(train_val_data):
    ...
    net = Parallel_network.parallel_model_cov(num_classes=2, dropout1=dropout_resnet, dropout2=dropout_tcnn, include_top=True, pth_1=None, pth_2=None)
    ...
    if os.path.exists(file_name):
        ...
        file.write("dropout_1:{}, dropout_2:{}\n".format(dropout_resnet, dropout_tcnn))
    ...
    for epoch in range(epochs):
        ...
        # for step, data in enumerate(train_loader):
            ...
            outputs = net(image.to(device), audio.to(device))
        ...
        with torch.no_grad():  # 下面不进行梯度计算
            ...
            for data_val in val_loader:
                # 获取验证集的图片和标签
                val_image, val_audio, val_labels = data_val
                # 前向传播
                outputs = net(val_image.to(device), val_audio.to(device))
            ...
            if val_loss[-1] >= min_val_loss + 0.1:
                if (epoch + 1) - epoch_num >= 4:
                    ...
                    # 保存新的字典到.pth文件
                    torch.save(weights_dict, savename)
                    break
    ...
    # 初始化网络
    # net = ResNet.resnet18(num_classes=2, include_top=True)
    net = Parallel_network.parallel_model_cov(num_classes=2, include_top=True)
    ...
    with torch.no_grad():
        ...
        for data_test in test_loader:
            # 获取测试集的图片和标签
            test_images, test_audio, test_labels = data_test
            #  前向传播
            outputs = net(test_images.to(device), test_audio.to(device))
            # 添加softmax层
